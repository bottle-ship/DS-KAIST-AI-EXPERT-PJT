...
def calculate_fid_given_paths(paths, inception_path, low_profile=False):
    ...
    local_session = tf.compat.v1.Session(config=config)
    ...
    local_session.__del__()

    return fid_value


def create_realdata_stats(x, output_path):
    config = tf.compat.v1.ConfigProto()
    config.gpu_options.allow_growth = True
    config.gpu_options.per_process_gpu_memory_fraction = 0.333
    local_session = tf.compat.v1.Session(config=config)
    ...
    inception_path = check_or_download_inception(inception_path)  # download inception if necessary
    print("ok")
    ...
    with local_session as sess:
        sess.run(tf.compat.v1.global_variables_initializer())
        mu, sigma = calculate_activation_statistics(images, sess, batch_size=64)
...
def fid_with_realdata_stats(x, stats_path):
    ...
    with local_session as sess:
        sess.run(tf.compat.v1.global_variables_initializer())
        mu_gen, sigma_gen = calculate_activation_statistics(images, sess, batch_size=64)
